...
from momo.tableau.tableau import Tableau, execute_tableau
...
def test_tableau_and_expansion():
    ...
    assert tableau.node.tl_set == TlSet([Formula(("G", Atom("a"))), Atom("b")])
...
def test_tableau_or_expansion():
    ...
    assert tableau.node.tl_set == TlSet([Formula(("G", Atom("a")))])


def test_tableau_release_expansion():
    formula = Formula(("R", Formula(("G", Atom("a"))), Formula(("F", Atom("a")))))
    tableau = Tableau(formula)
...
def test_tableau_eventually_expansion():
    ...
    assert tableau.node.tl_set == TlSet([formula.add_operator("X")])


def test_tableau_until_expansion():
    formula = Formula(("U", Formula(("G", Atom("a"))), Formula(("F", Atom("a")))))
    tableau = Tableau(formula)
...
def test_tableau_until_expansion_fail():
    initial_formula = [
        Formula(("U", Formula(("G", Atom("a"))), Formula(("F", Atom("a"))))),
        Formula(("G", Atom("-a"))),
        Formula(("F", Atom("-a"))),
    ]
    ...
    until_formula, multiplicity = tableau.node.pop_formula()
    assert until_formula.is_until()
    can_expand_left, _ = tableau._until_expansion_left(until_formula)
    assert not can_expand_left
    can_expand_right, _ = tableau._until_expansion_right(until_formula)
    assert not can_expand_right


def test_tableau_next_stage_expansion():
    initial_formula = [Formula(("X", Atom("a"))), Formula(("X", Atom("b"))), Atom("b")]
    tableau = Tableau(initial_formula)
...
def test_tableau_next_stage_expansion_and():
    ...
    can_expand, new_node = tableau._next_stage()
    assert can_expand
    assert new_node.tl_set == TlSet([Atom("b"), Atom("a"), Formula(("G", "a"))])
...
def test_tableau_next_stage_expansion_and_2():
    ...
    can_expand, new_node = tableau._next_stage()
    assert can_expand
    assert new_node.tl_set == TlSet([Atom("a"), Atom("a"), Formula(("G", "a"))])
...
def test_basic_closed_tableau():
    initial_formula = [Formula(("X", Atom("-a"))), Formula(("G", Atom("a")))]
...
def test_basic_tableau():
    initial_formula = [
        Formula(
            ("G", Formula(("&", frozenset([Atom("a"), Formula(("G", Atom("a")))]))))
        ),
        Formula(("X", Atom("-a"))),
        Atom("b"),
    ]
