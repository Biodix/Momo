...
class SatSolver:
    ...
    def z3_sat_translation(self, formula):
        if not(formula.is_and() or formula.is_or()):
            if formula not in self.tableau.closure.formula_to_sat:
                index= str(int(len(self.tableau.closure.formula_to_sat)/2 + 1))
                self.tableau.closure.formula_to_sat[formula] = index
                self.tableau.closure.formula_to_sat[formula.neg().nnf()] = '-' + index
